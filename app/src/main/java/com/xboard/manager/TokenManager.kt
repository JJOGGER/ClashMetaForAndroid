...
object TokenManager {
    ...
    fun saveToken(token: String?, email: String?, password: String?) {
        ...
        MMKVUtil.getInstance()
            .setValue(KEY_EXPIRE_TIME, System.currentTimeMillis() + 30 * 24 * 60 * 60 * 1000)
    }
    ...
    fun clearToken() {
        MMKVUtil.getInstance().clear(arrayOf(KEY_TOKEN, KEY_USER_PWD, KEY_USER_EMAIL, KEY_EXPIRE_TIME))
    }
}
